<?php
/**
 * Sharepoint Sistemas Colaborativos
...
// Libreria para resaltar onmouseover
$use_highlightjs = true;
// Stilo
$highlightjs_style = 'vs';
// Zona para correcta visualización de timestamps de archivos y carpetas
$default_timezone = 'America/Argentina/Buenos_Aires'; // UTC-3
// Directorio base
$root_path = $_SERVER['DOCUMENT_ROOT'].'/sharepoint/base_dir';
// Ubicación del Sharepoint
$root_url = '/sharepoint/base_dir';
$http_host = $_SERVER['HTTP_HOST'];
// Codificaciones y formatos de fecha
...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        if (isset($auth_users[$_POST['fm_usr']]) && $_POST['fm_pwd'] === $auth_users[$_POST['fm_usr']]) {
            ...
            fm_set_msg('Has iniciado sesión');
            fm_redirect(FM_SELF_URL . '?p=');
        } else {
            unset($_SESSION['logged']);
            fm_set_msg('Contraseña incorrecta', 'error');
            fm_redirect(FM_SELF_URL);
        }
    } else {
        // formulario login
        ...
        <div class="path">
            <form action="" method="post" style="margin:10px;text-align:center">
                ...
                <input type="password" name="fm_pwd" value="" placeholder="Contraseña" required>
...
if (isset($_GET['del'])) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Directorio <b>%s</b> eliminado' : 'Archivo <b>%s</b> eliminado';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? 'Directorio <b>%s</b> no eliminado' : 'Archivo <b>%s</b> no eliminado';
            fm_set_msg(sprintf($msg, fm_enc($del)), 'error');
        }
    } else {
        fm_set_msg('Nombre de archivo o directorio incorrecto', 'error');
    }
...
// Crear directorio
if (isset($_GET['new'])) {
    $new = strip_tags($_GET['new']); //strip_tags se utiliza para remover carácteres extraños
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            ...
            fm_set_msg(sprintf('Directorio <b>%s</b> ya existe', fm_enc($new)), 'alerta');
        } else {
...
if (isset($_GET['copy'], $_GET['finish'])) {
    ...
    // ruta vacía
    if ($copy == '') {
        fm_set_msg('No se definio origen', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                fm_set_msg(sprintf('Movido de <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg('Ya existe archivo o directorio en este sitio', 'alerta');
            } else {
                fm_set_msg(sprintf('Error mientras se movia de <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('Copiado de <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('Error mientras se copiaba de <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('Direcciones no deben ser iguales', 'alerta');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}

// copia masiva de archivos / carpetas
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if ($path == $copy_to_path) {
        fm_set_msg('Las rutas no deben ser iguales', 'alerta');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    if (!is_dir($copy_to_path)) {
        if (!fm_mkdir($copy_to_path, true)) {
            fm_set_msg('No se pudo crear directorio destino', 'error');
            fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
        }
    }
    // esta seleccionado check mover?
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? 'Archivos y directorios movidos' : 'Archivos y directorios copiados';
            fm_set_msg($msg);
        } else {
            $msg = $move ? 'Error al mover items' : 'Error al copiar items';
            fm_set_msg($msg, 'error');
        }
    } else {
        fm_set_msg('Nada seleccionado', 'alerta');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('Renombrado de <b>%s</b> a <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('Error renombrando de <b>%s</b> a <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nombres no especificados', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['upl'])) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('Todos los archivos se cargaron a <b>%s</b>', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('No se cargaron archivos', 'alerta');
    } else {
        fm_set_msg(sprintf('Error en la carga de archivos. Archivos cargados: %s', $uploads), 'error');
    }
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('Se eliminaron archivos y directorios seleccionados');
        } else {
            fm_set_msg('Error al eliminar items', 'error');
        }
    } else {
        fm_set_msg('Nada seleccionado', 'alerta');
    }
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button class="btn"><i class="icon-apply"></i> Cargar</button> &nbsp;
...
if (isset($_POST['copy'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p class="break-word">Directorio origen: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
                <label for="inp_copy_to">Directorio destino:</label>
...
if (isset($_GET['copy']) && !isset($_GET['finish'])) {
    ...
    <div class="path">
        <p><b>Copiando</b></p>
        <p class="break-word">
            Direccion origen: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Direccion destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="icon-apply"></i> Copiar</a></b> &nbsp;
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            Tamaño de archivo: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
            MIME-type: <?php echo $mime_type ?><br>
            <?php
            // Información de archivo tipo imagen
            if ($is_image) {
                $image_size = getimagesize($file_path);
                echo 'Dimensiones: ' . (isset($image_size[0]) ? $image_size[0] : '0') . ' x ' . (isset($image_size[1]) ? $image_size[1] : '0') . '<br>';
            }
            // Información de archivo tipo texto
            if ($is_text) {
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="icon-download"></i> Descargar</a></b> &nbsp;
...
fm_show_header(); // SECCIÓN CABECERA
fm_show_nav_path(FM_PATH); // SECCIÓN NAVEGACIÓN / ruta actual
...
<table><tr>
<th style="width:3%"><label><input type="checkbox" title="Invertir seleccion" onclick="checkbox_toggle()"></label></th>
<th>Nombre</th><th style="width:10%">Tamaño</th>
<th style="width:12%">Modificado</th>
<th style="width:13%"></th></tr>
<?php
// Link a directorio a carpeta principal
if ($parent !== false) {
    ?>
...
<td>Directorio</td><td><?php echo $modif ?></td>
<td>
<a title="Eliminar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Eliminar directorio?');"><i class="icon-cross"></i></a>
...
<a title="Link directo" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="icon-chain"></i></a>
</td></tr>
    <?php
    flush();
}
...
<td>
<a title="Eliminar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Eliminar archivo?');"><i class="icon-cross"></i></a>
<a title="Renombrar" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
...
if (empty($folders) && empty($files)) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>Carpeta vacía</em></td></tr>
<?php
} else {
    ?>
...
<a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> Invertir seleccion</a></p>
<p><input type="submit" name="delete" value="Eliminar" onclick="return confirm('Eliminar directorios y archivos seleccionados?')">
...
/**
 * Eliminar archivo / directorio (recursivo)
 * @param string $ruta
 * @return bool
 */
...
/**
 * Copiar archivos o directorios (recursivamente).
 * @param string $ruta
 * @param string $destino
 * @param bool $upd Actualizar archivos
 * @param bool $force Forzar creación
 * @return bool
 */
...
/**
 * Obtener tamaños de archivo pretty B, Kb, Mb, Gb, TB
 * @param int $size
 * @return string
 */
...
/**
 * Verificar si string esta en UTF-8
 * @param string $string
 * @return int
 */
...
/**
 * Convertir nombre de archivo a UTF-8 en Windows
...
/**
 * Obtener extensiones de tipo imagen
 * @return array
 */
...
/**
 * Obtener extensiones de tipo texto
 * @return array
 */
...
/**
 * Obtener tipos mime de archivos de texto
 * @return array
 */
...
/**
 * Nombres de archivo de texto sin extensión
 * @return array
 */
...
<a title="Cargar archivos" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="icon-upload"></i></a>
<a title="Crear directorio" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="icon-folder_add"></i></a>
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
        $root_url = "<a href='?p='><i class='icon-home' title='" . FM_ROOT_PATH . "'></i></a>  Directorio Raíz";
...
<title>Sistemas Colaborativos - Sharepoint</title>
<style>
...
/**
 * Mostrar sección pie de página
 */
function fm_show_footer()
{
    ?>
<p class="center"><small><a href="https://github.com/alexantr/filemanager" target="_blank">Sistemas Colaborativos - Sharepoint</a></small></p>
</div>
<script>
function newfolder(p){var n=prompt('Nuevo nombre de directorio','directorio');if(n!==null&&n!==''){window.location.search='p='+encodeURIComponent(p)+'&new='+encodeURIComponent(n);}}
...
/**
 * Mostrar Sección Visor de Imágen
 * @param string $img
 */
